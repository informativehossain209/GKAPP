...
public class MainActivity extends Activity {
    ...
    private static final String APP_URL = "https://g-happk.vercel.app/ops-cf3aed4d1274";
    ...
    //
    // IMPORTANT: this must NEVER crash the app. Firebase only initializes
    // successfully once google-services.json has been added and the APK
    // rebuilt (see PUSH_NOTIFICATIONS.md, step 2). Before that's done — or
    // if Google Play Services is missing/outdated on a device, or FCM is
    // briefly unreachable — every call below is guarded so the app still
    // opens and works normally; push notifications simply aren't active yet.
    private void setUpPushNotifications() {
        try {
            ...
            // Notification channel setup failing should never block app launch.
        }

        // FirebaseApp only exists once google-services.json was present at
        // build time. If it isn't, calling FirebaseMessaging.getInstance()
        // throws IllegalStateException and (previously) crashed the app on
        // every launch. Skip Firebase entirely until it's actually set up.
        ...
        try {
            ...
            FirebaseMessaging.getInstance().getToken()
                    ...
                    .addOnFailureListener(e -> { /* no token yet — fine, just no push targeting */ });
        } catch (Exception e) {
            // Any unexpected Firebase/Play-Services error must not crash the app.
        }
    }
