# 🔔 Push Notification Protocol — Axion DMS App
...
1. Go to **https://console.firebase.google.com** → **Add project** →
   name it e.g. `axion-dms` → skip Google Analytics (optional) → Create.
...
3. Still in Project settings → **General** tab → under "Your apps",
   ...
   - Android package name: `com.axion.dms` (must match exactly)
...
> **Already have a Firebase project from the older app (`com.miron.electronics`)?**
> You can reuse the same project — just click **Add app → Android** again and
> register `com.axion.dms` as a *second* app, then download that app's own
> `google-services.json`. Do **not** reuse the old one; its package name
> won't match and the build will fail.
> Heads-up: both apps subscribe to the same `all_users` topic, so if both are
> installed on one phone and you send to `all_users`, that phone gets the
> notification twice. To avoid it, use a separate Firebase project for this
> app, or change `FCM_BROADCAST_TOPIC` in `MainActivity.java` (and the topic
> your backend sends to) to something like `axion_all_users`.
...
`https://YOUR-APP.vercel.app/api/send-push`.
...
```bash
curl -X POST https://YOUR-APP.vercel.app/api/send-push \
  -H "Content-Type: application/json" \
  -H "x-push-secret: <PUSH_SECRET>" \
  -d '{"title":"নতুন অফার! 🔥","body":"আজ সব প্রোডাক্টে ২০% ছাড়","url":"https://YOUR-APP.vercel.app/offers"}'
```
...

{
  "message": {
    ...
    "data": {
      ...
      "url": "https://YOUR-APP.vercel.app/offers"
    },
    "android": { "priority": "high" }
  }
}
